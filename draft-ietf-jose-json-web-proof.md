...
JWTs [@RFC7519] are one of the most common representations for identity
...
in addition to traditional signatures.
...
All of these follow the same pattern of taking multiple claims (a.k.a.,
...
proofs of knowledge of the claims.

> Editor's Note: This draft is still early and incomplete. There will be
...
binding:
: A mechanism, indicated in an issued JWP, for how to verify a
  presentation was created by the intended holder.

linkability:
: The property where multiple presentations may be correlated to a
  ...
  leak unintended knowledge.

unlinkability:
: The property of issuance and presentation algorithms and of
  application usage, where one presentation can only be correlated with
  other presentations based on holder-disclosed information.
...
Signed Message [@RFC9052], with the addition that it can contain
...
The intent of JSON Web Proof is to establish a common container format
...
any algorithm-specific capabilities.
...
(#critDef), all Header Parameters not defined by this specification MUST
be ignored when not understood.
...
parameters having the same label.
...
The following Header Parameter names for use in JWPs are registered in
...
secured the content.  `alg` values should either be registered in the
...
signal a change of key to recipients.
...
The `typ` (type) Header Parameter is used by JWP applications to declare
the media type [@IANA.MediaTypes] of this complete JWP.  This is
...
can also be used by applications, including those using the `+jwp` media
...
The `iss` (issuer) Header Parameter identifies the principal that issued
...
The `aud` (audience) Header Parameter identifies the recipients that the
...
a value in the `aud` Header Parameter when this Header Parameter is
present, then the JWP MUST be rejected.
...
the JWP has one audience, the `aud` value MAY be a single case-sensitive
...
The `nonce` (nonce) Header Parameter is used to associate protocol state
...
New Header Parameters should be introduced sparingly, as they can result
in non-interoperable JWPs.
...
labels (#RegisteredHeaderParameterLabels) or Public Header Parameter
names (#PublicHeaderParameterName).) Unlike Public Header Parameter
...
may choose to omit payload information on a slot-by-slot basis.
...
Payloads are represented and processed as individual octet strings and
...
application or other specifications.
...
protection of that payload in the combined proof value.

### Issuer Proof

The issuer proof is one or more octet strings that are opaque to
...
The presentation has one or more payload slots, each of which either
...
* (Non-)Membership: Contains a particular role, Credential is not
  considered revoked.
...
The presentation proof is one or more octet strings that are opaque to
...
form by concatenating the BASE64URL encoded values using the `~`
character.

Individual payload slots are allowed to have their payload data be
...
`~` characters in the concatenated form.

If a payload or proof value was a zero-length octet string, it does not
get output as its zero-length BASE64URL-encoded form but as a single `_`
...
If an individual payload has been omitted, it is represented by the CBOR
value `nil`. Payloads MUST be included unless the application is using
detached payloads, which is represented by setting the `payloads` value
as `nil`.
...
or convention, in which case the `cty` value MAY be omitted.
...
The recipient is expected to perform some sequence of steps defined by
...
: The Header Parameter usage locations, which should be one or more of
  the values `Issued` or `Presented`.  Other values may be used with the
...
* Header Parameter Usage Location(s): Issued
* Change Controller: IETF
* Specification Document(s): (#holder_presentation_algorithm_keyDef) of
  this specification
...
* Encoding considerations: 8bit; application/jwp values are encoded as a
  series of base64url-encoded values (some of which may be the empty
  string) separated by period ('.') and tilde ('~') characters
* Security considerations: See (#SecurityConsiderations) of this
  specification
...
* Person & email address to contact for further information: Michael B.
  Jones, michael_b_jones@hotmail.com
...
* Required parameters: n/a
* Optional parameters: n/a
* Encoding considerations: 8bit; application/cwp values are represented
  as a CBOR data item.
* Security considerations: See (#SecurityConsiderations) of this
  specification
* Interoperability considerations: n/a
...
* Encoding considerations: binary; JWP values are encoded as a series of
...
* +suffix: +cwp
* References: (#CompactSerialization) of this specification
* Encoding considerations: 8bit; CWP values are represented as a CBOR
  data item.
* Interoperability considerations: n/a
* Fragment identifier considerations: n/a
* Security considerations: See (#SecurityConsiderations) of this
...
* (CDDL) `header_map`, `label` and `value` definitions imported from
  RFC 9052
* Receive examples of additional types of algorithm-specific proofs from
  JPT
...
* Deemphasis of JSON in some parts of the document to represent CBOR
  alternatives
* Rewrite Header Parameter parsing requirements for JSON to emphasize
  preference to fail on duplicate headers (to match CBOR behavior).
  Last-encountered remains as an option to match ECMA definition.
...
* Update BBS algorithm example in JSON serialization to show the proof
  as an array with a single octet string.
...
* Specified that algorithms may supply multiple octet strings for the
  proof, which are separated by `~` characters in the compact
  serialization.
