...
This example uses the Single-Use Algorithm as defined in JSON Proof Algorithms to create a JSON Proof Token.  It demonstrates how to apply selective disclosure using the widely adopted NIST curves with ECDSA.  Unlinkability is only acheived by using each JWP one time, as multiple uses are inherently linkable via the traditional signature embedded in the proof.

To begin we need two EC keys for Single-Use, one that represents the JPT Issuer's stable key, and the other is an ephemeral key generated by the Issuer just for this JWP.
...
Each payload must be individually encoded and then also signed by the Ephemeral Key.

The first payload is the string `"Doe"` with the octet sequence of `[]` and base64url-encoded as ` `.  When signed using the ephemeral private key, the resulting signature value is the octet sequence of `[]`.

The second payload is the string `"Jay"` with the octet sequence of `[]` and base64url-encoded as ` `.  When signed using the ephemeral private key, the resulting signature value is the octet sequence of `[]`.

The third payload is the string `"jaydoe@example.org"` with the octet sequence of `[]` and base64url-encoded as ` `.  When signed using the ephemeral private key, the resulting signature value is the octet sequence of `[]`.

The fourth payload is the string `42` with the octet sequence of `[]` and base64url-encoded as ` `.  When signed using the ephemeral private key, the resulting signature value is the octet sequence of `[]`.

The Issuer generates the initial JWP Proof value by first concatenating each payload's individual signature in order:

```json
[]
```

This larger octet sequence is then concatendated with the base64url-encoded Protected header and used as the signing input with the Issuer's stable key, resulting in a final signature.

The final proof value from the issuer is the concatenation of the payload signatures with the final stable signature, then base64url-encoded.
