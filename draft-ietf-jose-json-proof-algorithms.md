...
ipr = "trust200902"
workgroup="jose"
keyword = ["json", "jose", "zkp", "jwp", "jws", "jpa", "cbor", "cose"]
...
> Editor's Note: This draft is still early and incomplete.  There will
...
Stable Key:
: An asymmetric key-pair used by an issuer that is also shared via an
  out-of-band mechanism to a verifier to validate the signature.

Issuer Ephemeral Key:
: An asymmetric key-pair that is generated for one-time use by an issuer
  and never stored or used again outside of the creation of a single
  JWP.

Holder Presentation Key:
: An asymmetric key-pair that is generated by a holder and used to
...
Due to the nature of ZKPs, this specification also documents the subtle
but important differences in proof algorithms versus those defined by
the JSON Web Algorithms [@RFC7518].  These differences help support more
advanced capabilities such as blinded signatures and predicate proofs.

# Algorithm Basics

The four principal interactions that every proof algorithm MUST support
are [issue](#issue), [confirm](#confirm), [present](#present), and
[verify](#verify).
...
Each algorithm MAY support using additional input items options, such as
...
For COSE_Key values, a `proof_alg` parameter is likewise defined to
...
The Single Use (SU) algorithm is based on composing multiple traditional
...
It does not support unlinkability if the same JWP is presented multiple
...
The issuer MUST verify that the holder has possession of this key.  The
...
be used to sign each of the payloads of a single JWP and then discarded.
...
The holder derives a new proof as part of presentation.  The holder will
also use these components to generate a [presentation internal
representation](#presentation-internal-representation).  The number of
...
remain unlinkable across multiple presentations.
...
## Presentation Internal Representation {#presentation-internal-representation}

Some algorithms (such as Single use and MAC) use a holder key to provide
...
use this same form for consistency.
...
are then used as the input to generate a MAC that protects each payload.
...
When verifying a presentation, the shared secret will be unavailable so
...
to get the corresponding MAC of each payload:
...
fully-specified MAC algorithm variant.
...
See the [Presentation Header](#presentation-header)
...
The first proof component is the issuer signature over the Combined MAC
...
If a payload is disclosed, the corresponding proof component will be the
unique key.
...
Note that the second component of the issued JWP is a shared secret for
use by the holder to generate the unique keys used in the Combined MAC
Representation.  This MUST NOT be included in the presentation.
...
> Editor's Note: This will follow once the algorithms defined here have
> become more stable.
...
The implementation requirements of an algorithm may be changed over time
...
: Optional additional information clarifying the algorithm.  This may be
  used for example to document additional chosen parameters.
...
: The algorithm implementation requirements for JWP, which must be one of
  the words `Required`, `Recommended`, `Optional`, `Deprecated`, or
...
: References to a publication or publications in well-known
  ...
  document are exempt from the need to provide this information.
...
- Algorithm Description: Corresponds to a cipher suite identifier of
...
generated by the Issuer just for this JWP.

This is the Issuer's stable private key used in this example in the JWK
format:

<{{./fixtures/build/es256-issuer-private-key.jwk.wrapped}}
Figure: Issuer Private Key (ES256 in JWK)

This is the ephemeral private key used in this example in the JWK
format:

<{{./fixtures/build/es256-ephemeral-private-key.jwk.wrapped}}
Figure: Issuer Ephemeral Private Key (ES256 in JWK)

This is the Holder's presentation private key used in this example in
the JWK format:

<{{./fixtures/build/es256-holder-private-key.jwk.wrapped}}
...
<{{./fixtures/build/su-es256-issuer-header.json.wrapped}}
Figure: Issuer Header (SU-ES256, JSON)

<{{./fixtures/build/su-es256-issuer-header.base64url.wrapped}}
...
<{{./fixtures/build/su-es256-holder-header.json.wrapped}}
...
When signed and serialized, the CPT is represented by the following CBOR
(in hex):
...
<{{./fixtures/build/cpt-holder-header.edn}}>
...
<{{./fixtures/build/cpt-presentation-form.cbor.hex}}>
...
<{{./fixtures/build/bbs-private-key.jwk.wrapped}}
...
<{{./fixtures/template/bbs-holder-header.json}}
...
<{{./fixtures/build/mac-h256-issuer-header.json}}
Figure: Example Issuer Header
...
<{{./fixtures/build/mac-h256-issuer-payload-keys.json}}
...
<{{./fixtures/build/mac-h256-issuer-proof.json.wrapped}}
...
<{{./fixtures/build/mac-h256-holder-header.json.wrapped}}
Figure: Presentation Header
...
For each payload slot, the holder will provide one of two values as part
...
- Update of appendix describing MAC-H256 to now also be generated by the
  build system from a common set of code and templates.
- Update single use algorithm to use an array of octet values rather
  ...
  of a presentation and during verification.
- Update BBS algorithm description and examples to clarify the proof is
  an array with a single octet string.
- Update MAC algorithm to use an array of octet values for the proof,
  rather than requiring splitting an octet buffer into parts.
...
- SU and MAC families now use raw JWA rather than JWS and synthesized
  headers
- Change algorithms to not use base64url-encoding internally.
  ...
  used to represent those octets in JSON and compact serializations.
...
- Change algorithm from `BBS-DRAFT-5` to `BBS`, and from
...
- Remove prior `BBS-X` algorithm based on a particular implementation of
  earlier drafts.
