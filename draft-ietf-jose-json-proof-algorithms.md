...
The terms "JSON Web Signature (JWS)", "Base64url Encoding", "JOSE Header", "JWS Payload", "JWS Signature", and "JWS Protected Header" are defined by [@RFC7515].
The terms "JSON Web Signature (JWS)", "Base64url Encoding", "JOSE Header", "JWS Payload", "JWS Signature", and "JWS Protected Header" are defined by [@!RFC7515].

The terms "JSON Web Proof (JWP)", "Issued Form", "Presented Form", "Parameter", "Payload", "Proof",and "Protected Header" are defined by [@!I-D.ietf-jose-json-web-proof].
...
The Single Use (SU) algorithm is based on composing multiple traditional asymmetric signatures into a single Proof.  It enables a very simple form of selective disclosure without requiring any advanced cryptographic techniques.
...
The issuer's Ephemeral Key MUST be included in the issuer protected header via the Proof Key Parameter.

The holder's Presentation Key MUST be included in issuer protected header via the Presentation Key Parameter.

The issuer protected header is signed using the given JWA and the issuer's Stable Key.

### Payloads

Each Payload is processed in order, and signed using the given JWA using the issuer's Ephemeral Key.
...
To generate a new presentation, the holder first creates a presentation protected header that is specific to the verifier being presented to.  This header MUST contain one or more Parameters that, evaluated together, both the holder and verifier trust as being unique and non-replayable.
Use of the `nonce` Parameter is RECOMMENDED for this purpose.

This specification registers the `nonce` Parameter for the presentation protected header that contains a string value either generated by the verifier or derived from values provided by the verifier.  When present, the verifier MUST ensure the nonce value matches during verification.

The presentation protected header MAY contain other Parameters that are either provided by the verifier or by the holder.  These presentation Parameters SHOULD NOT contain values that are common across multiple presentations and SHOULD be unique to a single presentation and verifier.
...
The verifier MUST verify the issuer protected header octets against the first part in the proof using the issuer's Stable Key. It MUST also verify the presentation protected header octets against the second part in the proof value using the holder's Presentation Key, as provided in the Presentation Key Parameter.

With the headers verified, the Proof Key Parameter can then be used to verify each of the disclosed payload signatures.
...
The holder's presentation key MUST be included in the issuer's protected header using the Presentation Key Parameter.

### Issuer Setup

To use the MAC algorithm, the issuer must have a stable public key pair to perform signing.  To start the issuance process, a single 32-byte random Shared Secret must first be generated.  This value will be shared privately to the holder as part of the issuer's Proof value.
...
The first value in the presentation proof is the presentation signature. This is a signature over the presentation protected header, using the key specified by the Presentation Key Parameter in the issuer protected header.
...
for values of the JWP `alg` (algorithm) Parameter.
...
The Protected Header declares that the data structure is a JPT and the Proof Input is secured using the Single-Use ECDSA algorithm with the P-256 curve and SHA-256 digest.  It also includes the ephemeral public key, the Holder's presentation public key and list of claims used for this JPT.

<{{./fixtures/build/su-es256-issuer-protected-header.json.wrapped}}
Figure: Issuer Protected header (es256)

The Protected Header JSON is serialized (without the above whitespace added for readability) and uses UTF-8 encoding to convert into an octet string. This gives:
