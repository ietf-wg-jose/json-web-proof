...
generated and used for only one JWP.
...
The Holder's Presentation Key MUST be included via the Presentation Key
header parameter.
...
3. Extract the presentation key and holder presentation algorithm (if
   present) from the issuer protected header.
...
3. Verify the first proof component is a valid signature over the issuer
   protected header octets, using the issuer's stable key.
4. Extract the presentation key and holder presentation algorithm (if
   present) from the issuer protected header.
...
: The algorithm implementation requirements for JWP, which must be one
